# My HedgeX Portfolio Analysis

I built HedgeX to turn a basket of stocks into an interpretable portfolio backtest. I can set allocations, choose a time horizon, compare the resulting fund with the SPDR S&P 500 ETF (`SPY`), and trace the result back to individual holdings. The screenshots below show why I think that matters: instead of stopping at a large headline return, I can investigate benchmark-relative performance, downside risk, market sensitivity, and concentration.

> **Snapshot, not a recommendation.** The values below describe the portfolio and 10-year window shown in the screenshots. They are historical, sensitive to the selected dates and weights, and do not include taxes, fees, slippage, or trading constraints. “Alpha” in this interface is arithmetic outperformance, not regression-based or CAPM alpha.

## Portfolio setup and headline results

![HedgeX fund builder and performance summary](docs/assets/hedgex-fund-overview.png)

I entered 20 weighted holdings totaling 100%. HedgeX found sufficient history for 19 of them and compared the available basket with `SPY` over the same 10-year period. In this snapshot:

The controls at the top let me define the experiment. **Quick Presets** load example baskets; **Holdings** lists each requested ticker and target weight; **Equal Weight** divides the allocation evenly; **Total weight** confirms whether my allocations form a complete 100% portfolio; **Time Range** sets the historical window; and **Run Comparison** applies those choices against the displayed benchmark. A ticker chip is an intended position, not proof that the security was ultimately included—data validation happens when I run the comparison.
...
| Holdings analyzed | **19 of 20 requested** | At least one ticker I requested did not have usable data across the selected window, so HedgeX excluded it from the comparison. |
...
The distinction between percentage points and percent is important to me. The displayed `+329.6%` alpha is a **329.6-percentage-point spread** between two cumulative returns. It does not mean my portfolio earned 329.6% more on a multiplicative basis, and it is not risk-adjusted alpha.
...
| Weight | The holding's share of my **analyzed** portfolio. When a requested ticker lacks data, I rescale the remaining weights to total approximately 100%; small differences can remain because of display rounding. |
...
The setup screenshot shows DDOG at 14.43%, while my analyzed table shows 16.7%. The numbers indicate that ATZ—the 13.4% requested position—was excluded and the remaining holdings were renormalized. I calculate the new DDOG weight as:
...
That rounds to the displayed `16.7%`. Insufficient full-period history is a likely reason for ATZ's exclusion, but I would want the interface to report the exact cause. The rescaling is useful, but it also means the portfolio I tested is not identical to the one I initially requested.

## What I learned from the visualization

### 1. The selected backtest shows large—but highly concentrated—outperformance

My simulated fund beat SPY by 329.6 percentage points in the selected window. However, ANET contributed approximately **351.1 percentage points**. I calculated its share of the fund's total return as:
...
This means five of the 19 analyzed stocks generated about 84.7% of my portfolio's cumulative return.

Under my dashboard's contribution convention, removing ANET's contribution leaves:
...
My central conclusion is therefore not simply “this portfolio generates alpha.” It is: **a small number of exceptional winners generated nearly all of the observed edge.** I see that as valuable evidence, but also as a warning about repeatability and concentration risk.

### 2. Higher return came with materially higher market exposure

The approximate beta of 2.22 and maximum drawdown of −32.6% show me that the headline return was not free. My portfolio behaved like an aggressive equity basket and experienced meaningful losses along the way. The cumulative-alpha chart also contracts during several intervals, indicating that my relative performance was regime-dependent rather than uniformly positive.

### 3. Benchmark-relative performance and portfolio contribution answer different questions

`vs SPY` tells me whether an individual stock outperformed the benchmark. `Fund contribution` tells me how much that stock actually moved my portfolio after accounting for its weight. For holding `i`, I use:
...
Because HedgeX analyzed only 19 of my 20 requested holdings, I reweighted the surviving positions upward. This avoids leaving 13.4% of the model in unexplained cash, but it can introduce survivorship and availability bias. In a research-quality version, I would record which ticker was dropped, why it was dropped, the first common tradable date, and whether I rebalanced the portfolio.

## How I could provide value to QMI

[Quantitative Market Intelligence (QMI)](https://usitqmi.com/) describes its mission as teaching students to create, analyze, visualize, and use financially relevant datasets, and to turn that work into investible insights. I see HedgeX supporting that mission as:
...
My strongest next quantitative extensions would be CAGR, annualized volatility, Sharpe and Sortino ratios, tracking error, information ratio, rolling beta, sector/factor exposures, turnover, and confidence intervals. Those additions would help me and QMI distinguish raw excess return from risk-adjusted and statistically credible performance.

## How I could provide value to USIT as a whole

[USIT](https://www.texasusit.org/usit-foundation) emphasizes fundamental investing alongside data science. I would use HedgeX as a bridge between those disciplines rather than present it as a stand-alone stock picker:
...
For USIT, I do not think the most meaningful output is the `+643.8%` headline by itself. The value is my ability to ask better portfolio questions: **Was the result diversified? Was it worth the risk? Which thesis actually drove the outcome? Would the conclusion survive a different start date, benchmark, or rebalancing rule?** With HedgeX, I can make those questions visible and give analysts a foundation for answering them systematically.
...
I do not think these limitations reduce HedgeX's usefulness. They define my path from a compelling educational visualization to a credible quantitative research platform.
